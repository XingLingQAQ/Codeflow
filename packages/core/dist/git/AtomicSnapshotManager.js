...
import { DEFAULT_SNAPSHOT_CONFIG, } from './AtomicSnapshotTypes.js';
export class AtomicSnapshotManager {
    constructor(storage, gitManager, config = {}) {
    ...
    async findSnapshotByGitHash(gitHash) {
        ...
        if (snapshot.conversation.checksum) {
            ...
            if (this.conversationProvider) {
                ...
                if (messages.length === snapshot.conversation.messageCount) {
                    ...
                    // 当前消息更少，可能数据丢失
                    result.errors.push(`Conversation message count mismatch: expected ${snapshot.conversation.messageCount}, got ${messages.length}`);
                }
            }
            else {
                // 无法验证，假设有效
                ...
                try {
                    ...
                    if (info.name === snapshot.memory.vector.collectionName) {
                        ...
                        if (info.count >= snapshot.memory.vector.chunkCount) {
                            ...
                            result.errors.push(`Vector chunk count decreased: expected >= ${snapshot.memory.vector.chunkCount}, got ${info.count}`);
                        }
                    }
                    else {
                        result.errors.push(`Vector collection name mismatch: expected ${snapshot.memory.vector.collectionName}, got ${info.name}`);
                    }
                }
                catch {
                ...
                try {
                    ...
                    if (stats.tripleCount >= snapshot.memory.graph.tripleCount) {
                        ...
                        if (stats.entityCount >= snapshot.memory.graph.entityCount) {
                            ...
                            result.errors.push(`Graph entity count decreased: expected >= ${snapshot.memory.graph.entityCount}, got ${stats.entityCount}`);
                        }
                    }
                    else {
                        result.errors.push(`Graph triple count decreased: expected >= ${snapshot.memory.graph.tripleCount}, got ${stats.tripleCount}`);
                    }
                }
                catch {
    ...
    async pruneSnapshots(keepCount) {
    ...
    async rollbackVector(targetState) {
        if (!this.vectorStore)
            return;
        // 获取所有 chunks 并按时间戳过滤
        // 策略：删除目标时间戳之后添加的所有数据
        try {
            ...
            if (info.count > targetState.chunkCount) {
                // 无法精确删除，记录警告
                console.warn(`Vector rollback: cannot precisely delete chunks. Current: ${info.count}, Target: ${targetState.chunkCount}`);
            }
        }
    }
    async rollbackGraph(targetState) {
        if (!this.tripleStore)
            return;
        // 查询并删除目标时间戳之后的三元组
...
//# sourceMappingURL=AtomicSnapshotManager.js.map