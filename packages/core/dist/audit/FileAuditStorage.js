 ...
 * 默认文件存储配置
 */
export const DEFAULT_FILE_STORAGE_CONFIG = {
    logDir: './audit-logs',
    filePrefix: 'audit',
...
export class FileAuditStorage {
    ...
    async readEntryFromFile(file, lineNumber) {
        const fileStream = fs.createReadStream(file);
        const rl = readline.createInterface({
            input: fileStream,
            crlfDelay: Infinity,
    ...
    async readEntriesFromFile(file) {
        const entries = [];
        const fileStream = fs.createReadStream(file);
        const rl = readline.createInterface({
            input: fileStream,
    ...
    matchesQuery(entry, query) {
        ...
        if (query.endTime && entry.timestamp > query.endTime)
            return false;
        if (query.eventTypes?.length && !query.eventTypes.includes(entry.eventType))
            return false;
        if (query.severities?.length && !query.severities.includes(entry.severity))
            return false;
 ...
 * 创建文件审计存储实例并初始化
 */
export async function createFileAuditStorage(config) {
