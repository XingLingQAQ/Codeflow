...
export class GeminiCLIAdapter {
    ...
    async stream(command, onChunk, options) {
        const payload = await this.applyBeforeSendHooks(command);
        const processId = await this.processManager.spawn(this.config.geminiPath, this.buildPromptArgs(payload.prompt, payload.model, true), {
    ...
    buildPromptArgs(prompt, model, streaming = false) {
