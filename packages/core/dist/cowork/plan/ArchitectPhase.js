...
export class TechResearcher extends EventEmitter {
     ...
     * 从愿景提取研究主题
     */
    extractTopicsFromVision(vision) {
        const topics = [];
        // 从目标提取
    ...
    generateStaticResearch(topic, context) {
        return {
...
export class ArchitectureDesigner extends EventEmitter {
     ...
     * 设计架构
     */
    async design(vision, constraints, research) {
    ...
    generateStaticArchitecture(vision, constraints, research) {
        // 从研究结果提取技术栈
        const techStack = research.map(r => ({
            category: 'framework',
            name: r.topic,
        ...
        // 从愿景生成组件
        const systemComponents = vision.goals.map((goal, i) => ({
            name: `Component${i + 1}`,
            type: 'module',
            responsibility: goal,
            interfaces: [],
        ...
        const integrations = [];
        // 安全架构
        const securityArchitecture = this.config.includeSecurityAnalysis
            ? {
        ...
        // 可扩展性计划
        const scalabilityPlan = this.config.includeScalabilityPlan
            ? {
    ...
    generateMarkdown(architecture) {
        ...
        for (const tech of content.techStack) {
            ...
            if (tech.version)
                lines.push(`- **Version**: ${tech.version}`);
...
export class RoadmapGenerator extends EventEmitter {
     ...
     * 生成 Roadmap
     */
    async generate(vision, architecture) {
    ...
    generateStaticRoadmap(vision, architecture) {
        const milestones = [];
        const prdQueue = [];
        const timeline = [];
        const dependencies = [];
...
export class ArchitectPhase extends EventEmitter {
     ...
     * 执行架构设计阶段
     */
    async execute(vision, constraints) {
...
//# sourceMappingURL=ArchitectPhase.js.map