...
export class GodCommittee extends EventEmitter {
    ...
    canAutoApprove(proposal) {
        if (!this.config.autoApproveBelow)
            return false;
     ...
     * 计算投票结果
     */
    calculateResult(proposal, votes) {
        let totalWeight = 0;
        let approveWeight = 0;
        let rejectWeight = 0;
     ...
     * 生成结果摘要
     */
    generateResultSummary(outcome, approvalRate, votes) {
     ...
     * 进行风险评估
     */
    async assessRisk(proposalId, assessedBy) {
        const proposal = this.proposals.get(proposalId);
        if (!proposal) {
            throw new Error(`Proposal ${proposalId} not found`);
        }
    ...
    calculateOverallRisk(factors) {
        ...
        if (maxScore >= 9 || avgScore >= 7)
            return 'critical';
        if (maxScore >= 7 || avgScore >= 5)
            return 'high';
        if (maxScore >= 5 || avgScore >= 3)
            return 'medium';
        return 'low';
    }
    /**
     * 生成缓解计划
     */
    generateMitigationPlan(factors) {
        const steps = [];
     ...
     * 获取风险评估
     */
    getRiskAssessment(proposalId) {
        return this.riskAssessments.get(proposalId);
    }
    /**
     * 生成监督报告
     */
    async generateReport(sessionId, startTime, endTime) {
        const now = Date.now();
        ...
        // 初始化统计
        const types = ['architecture', 'technology', 'implementation', 'security', 'performance', 'resource', 'scope', 'priority'];
        const severities = ['critical', 'high', 'medium', 'low'];
        for (const t of types)
            statistics.byType[t] = 0;
     ...
     * 识别关注点
     */
    identifyConcerns(decisions, statistics) {
        const concerns = [];
        // 检查高拒绝率
     ...
     * 生成建议
     */
    generateRecommendations(concerns, statistics) {
     ...
     * 记录审计
     */
    async recordAudit(proposalId, action, actor, details) {
        const record = {
...
//# sourceMappingURL=GodCommittee.js.map