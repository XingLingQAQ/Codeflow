...
export class TaskTypeRouter extends EventEmitter {
     ...
     * 路由任务到模型
     */
    route(taskType, availableModels) {
...
export class UsageTracker extends EventEmitter {
     ...
     * 记录使用
     */
    record(modelId, taskType, inputTokens, outputTokens, sessionId) {
    ...
    getStatistics(startTime, endTime) {
        ...
        if (premiumCost) {
            const premiumTotalCost = (stats.totalInputTokens / 1000) * premiumCost.inputCostPer1k +
...
export class CostOptimizer extends EventEmitter {
     ...
     * 选择最优模型
     */
    selectModel(taskType, availableModels) {
        // 检查预算
     ...
     * 记录使用
     */
    recordUsage(modelId, taskType, inputTokens, outputTokens, sessionId) {
