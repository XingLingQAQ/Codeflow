...
export class ContextRestorer extends EventEmitter {
     ...
     * 加载摘要
     */
    async loadSummary(userId, relativePath) {
...
export class SessionJournal extends EventEmitter {
    constructor(config = {}, summaryGenerator) {
        super();
        this.currentSession = [];
    ...
    generateDefaultSummary() {
        ...
        const keyPoints = userEntries.slice(0, 3).map(e => e.content.length > 100 ? e.content.substring(0, 97) + '...' : e.content);
        // 提取代码变更（从助手消息中查找文件引用）
        const codeChanges = [];
     ...
     * 恢复上下文
     */
    async restoreContext(userId, query) {
        return this.restorer.restore(userId, query);
    }
    /**
     * 获取最近日志
     */
    async getRecentJournals(userId, count = 5) {
        return this.indexManager.getRecent(userId, count);
    }
    /**
     * 搜索日志
     */
    async searchJournals(userId, query) {
