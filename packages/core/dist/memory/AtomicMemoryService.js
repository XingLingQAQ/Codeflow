...
class LocalVectorStore {
    ...
    search(query, options) {
        ...
        const sliced = this.sliceWithPagination(scored.map((item) => item.memory), context.offset, context.topK);
        return sliced;
    }
    ...
    computeScore(memory, query, keywords) {
    ...
    matchesFilter(memory, context) {
        if (context.sessionId && memory.sessionId !== context.sessionId)
            return false;
        if (context.folderId && memory.folderId !== context.folderId)
            return false;
        if (context.startAt !== undefined && memory.timestamp < context.startAt)
            return false;
        if (context.endAt !== undefined && memory.timestamp > context.endAt)
            return false;
        if (context.tags?.length) {
            const tagSet = new Set(memory.tags.map((tag) => tag.toLowerCase()));
            const required = context.tags.map((tag) => tag.toLowerCase());
            if (!required.every((tag) => tagSet.has(tag)))
                return false;
        }
        return true;
    }
    toSearchContext(options) {
        return {
    ...
    sliceWithPagination(items, offset, limit) {
        if (!items.length)
            return [];
...
export class AtomicMemoryService {
    constructor(config = {}) {
    ...
    async add(memory) {
        ...
        const created = await this.request(ATOMIC_MEMORY_PATH, {
            method: 'POST',
            body: payload,
        });
    ...
    async search(query, options) {
        ...
        const remote = await this.request(`${ATOMIC_MEMORY_PATH}/search${params}`, {
            method: 'GET',
        });
    ...
    async searchByTimeRange(start, end) {
    ...
    async update(id, updates) {
        ...
        const updated = await this.request(`${ATOMIC_MEMORY_PATH}/${encodeURIComponent(memoryId)}`, {
            method: 'PUT',
            body: payload,
        });
    ...
    async delete(id) {
        ...
        await this.request(`${ATOMIC_MEMORY_PATH}/${encodeURIComponent(memoryId)}`, {
            method: 'DELETE',
        });
    ...
    async getBySession(sessionId) {
        ...
        if (!normalizedSessionId)
            return [];
        const remote = await this.request(`${ATOMIC_MEMORY_PATH}/session/${encodeURIComponent(normalizedSessionId)}?limit=200&offset=0`, { method: 'GET' });
        const normalized = remote.map((item) => this.fromRemote(item));
        this.localStore.addMany(normalized);
        return normalized;
    }
    ...
    fromRemote(memory) {
        const wire = memory;
        const sessionId = typeof wire.sessionId === 'string'
            ? wire.sessionId
            : typeof wire.session_id === 'string'
                ? wire.session_id
                : '';
        const folderId = typeof wire.folderId === 'string'
            ? wire.folderId
    ...
    buildSearchParams(query, options) {
        ...
        if (options?.endAt && options.endAt > 0)
            params.set('end_at', String(options.endAt));
        const raw = params.toString();
        return raw ? `?${raw}` : '';
    }
    hasSufficientLocalScore(localItems, query) {
    ...
    keywordSimilarity(content, query) {
    ...
    applySearchPostFilter(items, options) {
        let result = [...items];
        if (options?.sessionId) {
    ...
    toUpdatePayload(updates) {
        ...
        if (updates.embedding !== undefined)
            payload.embedding = updates.embedding;
...
//# sourceMappingURL=AtomicMemoryService.js.map