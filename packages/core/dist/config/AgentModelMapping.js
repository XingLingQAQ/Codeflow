...
export class AgentModelMapping extends EventEmitter {
     ...
     * 获取 Agent 使用的模型 ID
     */
    getModelForAgent(agent, taskType) {
        const config = this.mappings.get(agent);
        if (!config)
            return undefined;
        // 检查任务类型覆盖
     ...
     * 获取 Agent 使用的模型定义
     */
    getModelDefinitionForAgent(agent, taskType) {
     ...
     * 设置 Agent 的任务类型覆盖模型
     */
    setTaskTypeOverride(agent, taskType, modelId) {
        if (!this.modelRegistry.hasModel(modelId)) {
            return false;
        }
        const config = this.mappings.get(agent);
     ...
     * 移除任务类型覆盖
     */
    removeTaskTypeOverride(agent, taskType) {
        const config = this.mappings.get(agent);
        if (!config?.taskTypeOverrides)
            return false;
     ...
     * 设置 Agent 的备用模型
     */
    setFallbackModelForAgent(agent, modelId) {
        if (!this.modelRegistry.hasModel(modelId)) {
            return false;
        }
        const config = this.mappings.get(agent);
    ...
    setMappings(mappings) {
        for (const [agent, modelId] of Object.entries(mappings)) {
            this.setModelForAgent(agent, modelId);
        }
    }
    /**
     * 重置为默认映射
        ...
        for (const agent of ALL_AGENTS) {
            ...
            if (config.taskTypeOverrides) {
                for (const [taskType, modelId] of Object.entries(config.taskTypeOverrides)) {
                    if (!this.modelRegistry.hasModel(modelId)) {
                        errors.push(`Task type override model not found for agent ${agent}, task ${taskType}: ${modelId}`);
                    }
                }
            }
            // 检查能力要求
                ...
                if (model) {
                    for (const cap of config.requiredCapabilities) {
                        if (!model.capabilities.includes(cap)) {
                            errors.push(`Model ${config.modelId} missing required capability '${cap}' for agent ${agent}`);
                        }
                    }
                }
            }
     ...
     * 获取适合指定 Agent 的模型列表
     */
    getSuitableModelsForAgent(agent) {
        const config = this.mappings.get(agent);
        if (!config?.requiredCapabilities) {
     ...
     * 估算所有 Agent 的总成本
     */
    estimateTotalCost(tokensPerAgent = 10000) {
        let totalCost = 0;
        for (const agent of ALL_AGENTS) {
            const model = this.getModelDefinitionForAgent(agent);
            if (model) {
                const cost = this.modelRegistry.estimateCost(model.id, tokensPerAgent, tokensPerAgent / 2);
                if (cost !== undefined) {
                    totalCost += cost;
                }
            }
...
//# sourceMappingURL=AgentModelMapping.js.map