...
export class ConfigManager extends EventEmitter {
    ...
    getConfigHierarchy() {
        const roleConfigs = {};
        for (const role of ['main', 'coder', 'sub']) {
            const config = this.roleConfigs.get(role);
            if (config) {
    ...
    detectConflicts() {
        ...
        for (const role of ['main', 'coder', 'sub']) {
            ...
            if (roleConfig.apiChannel !== 'default') {
                ...
                if (!channel) {
                    conflicts.push(`Role '${role}' references non-existent API channel '${roleConfig.apiChannel}'`);
                }
                else if (!channel.enabled) {
                    conflicts.push(`Role '${role}' references disabled API channel '${roleConfig.apiChannel}'`);
                }
            }
        }
        return conflicts;
