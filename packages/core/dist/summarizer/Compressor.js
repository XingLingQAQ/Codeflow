...
export class Compressor {
    ...
    async compress(context, config) {
        ...
        if (this.summaryAdapter && messages.length > preservedMessages.length) {
            const messagesToSummarize = messages.slice(0, messages.length - mergedConfig.preserveRecentMessages);
    ...
    async extractSkeleton(messages) {
        const entities = [];
        const decisions = [];
        const relations = [];
        // 实体提取模式
    ...
    async generateSummary(messages, config) {
    ...
    // ==================== 私有方法 ====================
    applyCompressionStrategy(messages, config) {
        const preserved = [];
        ...
        if (historicalMessages.length > 0) {
            const targetHistoricalTokens = Math.floor(config.maxTokens * config.targetRatio - this.tokenCounter.countMessages(preserved).total);
            if (targetHistoricalTokens > 0) {
                ...
                // 按原始顺序排列
                selectedHistorical.sort((a, b) => historicalMessages.indexOf(a) - historicalMessages.indexOf(b));
                // 插入到系统消息之后、最近消息之前
    ...
    generateLocalSummary(messages) {
        ...
        parts.push(`Statistics: ${userMessages.length} user messages, ${assistantMessages.length} assistant responses.`);
        return parts.join(' ');
    }
