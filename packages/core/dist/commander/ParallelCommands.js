...
export class ParallelCommands extends EventEmitter {
    ...
    async execute(commandName, params) {
        ...
        try {
            ...
            switch (commandName) {
                case 'parallel-start':
                    result = await this.executeParallelStart(params);
                    break;
                case 'parallel-status':
                    result = await this.executeParallelStatus(params);
                    break;
                case 'parallel-compare':
                    result = await this.executeParallelCompare(params);
                    break;
                case 'parallel-select':
                    result = await this.executeParallelSelect(params);
                    break;
                case 'parallel-merge':
                    result = await this.executeParallelMerge(params);
                    break;
                default:
                    result = {
                        success: false,
     ...
     */
    async executeParallelStart(params) {
     ...
     */
    async executeParallelStatus(params) {
        if (params.taskId) {
     ...
     */
    async executeParallelCompare(params) {
        const task = this.activeTasks.get(params.taskId);
        if (!task) {
            return {
                success: false,
        ...
        if (params.format === 'json') {
            ...
            const rows = solutions.map(s => {
                const total = metrics.reduce((sum, m) => sum + (s.scores[m] || 0), 0) / metrics.length;
                return `| ${s.id} | ${s.model} | ${metrics.map(m => s.scores[m] || 'N/A').join(' | ')} | ${total.toFixed(1)} |`;
            });
            output = `${header}\n${separator}\n${rows.join('\n')}`;
        }
        else {
            ...
            const rows = solutions.map(s => {
                ...
                return `${s.id.padEnd(14)} ${s.model.padEnd(18)} ${metrics.map(m => String(s.scores[m] || 'N/A').padEnd(12)).join('')}${total.toFixed(1)}${rec}`;
            });
            output = `${header}\n${'─'.repeat(80)}\n${rows.join('\n')}`;
        }
        return {
     ...
     */
    async executeParallelSelect(params) {
        const task = this.activeTasks.get(params.taskId);
        if (!task) {
            return {
                success: false,
     ...
     */
    async executeParallelMerge(params) {
        const task = this.activeTasks.get(params.taskId);
        if (!task) {
            return {
                success: false,
...
//# sourceMappingURL=ParallelCommands.js.map