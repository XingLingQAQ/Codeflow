...
export class ContextLoader {
    ...
    async loadContext(intent) {
        ...
        const scored = await Promise.all(intentFiles.map(async (filePath) => {
    ...
    async loadWithDependencies(filePath) {
    ...
    async readFileOrEmpty(filePath) {
        try {
    ...
    computeRelevance(queryKeywords, docKeywords) {
    ...
    resolveIntentDocPath(sourceFilePath) {
    ...
    resolveImportIntentPath(importPath, fromFile) {
        const projectRoot = path.resolve(this.config.projectRoot);
        let resolvedImport;
