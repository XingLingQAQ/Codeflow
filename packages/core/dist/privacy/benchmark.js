...
export class EncryptionBenchmark {
     ...
     * 测试指定数据量的加密/解密性能
     */
    async benchmarkEncryption(dataSize) {
     ...
     * 测试密钥派生性能
     */
    async benchmarkKeyDerivation(method) {
    ...
    static printReport(report) {
        ...
        for (const r of report.testSizes) {
            console.log(`${r.dataSizeLabel}\t\t${r.encryptOpsPerSec.toFixed(2)}\t\t${r.decryptOpsPerSec.toFixed(2)}\t\t${r.encryptThroughputMBps.toFixed(2)}\t\t${r.decryptThroughputMBps.toFixed(2)}`);
        }
