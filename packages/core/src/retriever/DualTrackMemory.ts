...
import {
  IDualTrackMemory,
  DualTrackSearchConfig,
  DualTrackSearchParams,
  DualTrackSearchResponse,
  DualTrackSearchResult,
  GraphSearchResult,
  ActivatedNode,
  DEFAULT_DUAL_TRACK_CONFIG,
} from './DualTrackTypes.js';
import { IVectorStore, VectorSearchResult } from '../memory/types.js';
import { ITripleStore, Triple, TripleNode, isLiteralValue } from '../samg/types.js';
import { ISemanticRetriever } from './types.js';

export class DualTrackMemory implements IDualTrackMemory {
  private config: DualTrackSearchConfig;
  ...
  async hybridSearch(params: DualTrackSearchParams): Promise<DualTrackSearchResponse> {
    const startTime = Date.now();
    const limit = params.limit || this.config.topK;
    const searchMode = params.searchMode || 'hybrid';

    let vectorResults: DualTrackSearchResult[] = [];
  ...
  async vectorSearch(query: string, limit: number = 10): Promise<DualTrackSearchResult[]> {
  ...
  async graphSearch(query: string, limit: number = 10): Promise<GraphSearchResult[]> {
    ...
    try {
      ...
      for (const entity of matchedEntities.slice(0, limit)) {
        ...
        // 计算相关性分数
        const labelMatch = queryTokens.filter(t =>
          entity.label.toLowerCase().includes(t)
      ...
      if (this.config.enableSpreadingActivation && matchedEntities.length > 0) {
        ...
        const activated = await this.spreadingActivation(
          seedIds,
          this.config.spreadingDepth,
          this.config.spreadingDecay
        );
  ...
  async spreadingActivation(
    ...
    const activated: Map<string, ActivatedNode> = new Map();
    ...
    for (let d = 0; d < depth; d++) {
      ...
      for (const node of currentLevel) {
        ...
        for (const neighbor of neighbors) {
          ...
          if (newActivation < 0.1) continue; // 阈值过滤
  ...
  private mergeResults(
    vectorResults: DualTrackSearchResult[],
    graphResults: GraphSearchResult[],
    params: DualTrackSearchParams
  ): DualTrackSearchResult[] {
    const merged: Map<string, DualTrackSearchResult> = new Map();
  ...
  private buildContentFromGraph(gr: GraphSearchResult): string {
    const parts: string[] = [];
