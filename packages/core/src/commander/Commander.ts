...
import {
  ...
  AgentConfig,
  AgentRole,
  CallCoderAgentParams,
  ConsultSubExpertParams,
  ToolCallResult,
  ContextGraftConfig,
  GraftedContext,
  ToolDefinition,
...
export class Commander implements ICommander {
  ...
  private eventHandlers: Map<CommanderEvent, CommanderEventHandler[]> = new Map();
  private hookManager?: HookManager;
  private maxNestingDepth: number;

  constructor(hookManager?: HookManager, maxNestingDepth = 5) {
  ...
  async callCoderAgent(params: CallCoderAgentParams): Promise<ToolCallResult> {
  ...
  async consultSubExpert(params: ConsultSubExpertParams): Promise<ToolCallResult> {
    const startTime = Date.now();
    const callId = this.generateCallId();
  ...
  async graftContext(
    ...
    const graftedContext: GraftedContext = {
      messages,
    ...
    return [
      {
        ...
        description:
          'Delegate a coding task to the Coder Agent. Use this for code generation, refactoring, debugging, or any programming-related tasks.',
        parameters: {
          type: 'object',
          properties: {
            task: {
      ...
      },
      {
        ...
        description:
          'Consult a domain-specific sub-expert for specialized knowledge. Use this for questions requiring deep expertise in a specific area.',
        parameters: {
          type: 'object',
          properties: {
            domain: {
              type: 'string',
              description:
                'The domain of expertise (e.g., "security", "performance", "architecture")',
            },
            question: {
              type: 'string',
  ...
  private pushCall(trace: CallTrace): void {
  ...
  private emit(event: CommanderEvent, data: unknown): void {
    const handlers = this.eventHandlers.get(event) || [];
  ...
  private buildCoderPrompt(params: CallCoderAgentParams): string {
  ...
  private buildSubExpertPrompt(params: ConsultSubExpertParams): string {
