...
const DEFAULT_CONFIG: PassiveRAGConfig = {
  topN: 5,
  threshold: 0.3,
...
export class PassiveRAGService {
  ...
  private readonly config: PassiveRAGConfig;
   ...
   * 排名越靠前，评分越高
   */
  private computeSemanticScore(rank: number, total: number): number {
    if (total <= 1) return 1;
    return 1 - rank / total;
  }
   ...
   * 越近的记忆评分越高
   */
  private computeTimeScore(timestamp: number, now: number): number {
