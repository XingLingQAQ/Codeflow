...
export class MemorySearchTool {
  ...
  getToolDefinition(): MemorySearchToolDefinition {
    return {
      name: 'search_memory',
      description:
        '搜索历史记忆。支持语义搜索、时间范围过滤和标签过滤。用于检索之前对话中的关键信息、用户偏好、技术决策等。',
      inputSchema: {
        type: 'object',
        properties: {
          query: {
   ...
   * 执行记忆搜索
   */
  async execute(params: MemorySearchParams): Promise<MemorySearchResult> {
    this.validateParams(params);

    const limit = params.limit && params.limit > 0 ? params.limit : 10;
    let memories: AtomicMemory[];

    if (params.timeRange) {
      const timeResults = await this.memoryService.searchByTimeRange(
  ...
  private validateParams(params: MemorySearchParams): void {
    if (!params) {
    ...
    if (params.timeRange) {
      if (
        typeof params.timeRange.start !== 'number' ||
