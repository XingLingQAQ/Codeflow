...
export class WorktreeManager extends EventEmitter {
   ...
   * 创建新的 Worktree
   */
  async createWorktree(name: string, options: CreateWorktreeOptions = {}): Promise<WorktreeInfo> {
    // 检查是否超过最大数量
   ...
   * 移除 Worktree
   */
  async removeWorktree(pathOrName: string, force: boolean = false): Promise<boolean> {
   ...
   * 锁定 Worktree
   */
  async lockWorktree(pathOrName: string, reason?: string): Promise<boolean> {
    const worktreePath = this.resolveWorktreePath(pathOrName);
   ...
   * 解锁 Worktree
   */
  async unlockWorktree(pathOrName: string): Promise<boolean> {
    const worktreePath = this.resolveWorktreePath(pathOrName);
   ...
   * 清理可修剪的 Worktrees
   */
  async cleanupPrunable(): Promise<number> {
    try {
   ...
   * 移除所有非主 Worktrees
   */
  async removeAllWorktrees(force: boolean = false): Promise<number> {
    let removed = 0;
   ...
   */
  async execInWorktree(pathOrName: string, command: string): Promise<string> {
    const worktreePath = this.resolveWorktreePath(pathOrName);
   ...
   * 获取 Worktree 的当前分支
   */
  async getWorktreeBranch(pathOrName: string): Promise<string | null> {
    try {
   ...
   */
  async getWorktreeCommit(pathOrName: string): Promise<string | null> {
    try {
   ...
   * 解析 Worktree 条目
   */
  private parseWorktreeEntry(entry: string): WorktreeInfo | null {
   ...
   */
  private findWorktreeByPath(targetPath: string): WorktreeInfo | undefined {
    // 先尝试直接查找
   ...
   * 解析 Worktree 路径
   */
  private resolveWorktreePath(pathOrName: string): string {
    // 如果是绝对路径，直接规范化返回
