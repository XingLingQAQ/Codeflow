...
export class MapAgent implements IMapAgent {
  private adapter?: ICliAdapter;
  private config: MapAgentConfig;
  ...
  async buildMap(messages: Message[], sessionId: string): Promise<CompressionMap> {
    ...
    if (this.config.extractEntities) {
      for (const entity of extraction.entities) {
        if (entity.importance < this.config.minImportance) continue;
    ...
    if (this.config.extractDecisions) {
      for (const decision of extraction.decisions) {
        if (decision.importance < this.config.minImportance) continue;
    ...
    // 构建决策骨架
    const skeleton: DecisionSkeleton = {
  ...
  mergeMap(existing: CompressionMap, newMap: CompressionMap): CompressionMap {
    ...
    const existingEdgeKeys = new Set(
      existing.edges.map((e) => `${e.source}-${e.target}-${e.type}`)
    );
    const newEdges = newMap.edges.filter(
  ...
  private async extractWithLLM(messages: Message[]): Promise<ExtractionResult> {
    if (!this.adapter) {
      return this.extractLocally(messages);
    }
  ...
  private extractLocally(messages: Message[]): ExtractionResult {
...
Respond in JSON format:
{
  ...
  private inferEntityType(entity: string, context: string): string {
    const lowerContext = context.toLowerCase();
    const lowerEntity = entity.toLowerCase();

    if (lowerContext.includes(`class ${lowerEntity}`)) return ENTITY_TYPES.CLASS;
    if (lowerContext.includes(`function ${lowerEntity}`)) return ENTITY_TYPES.FUNCTION;
    if (lowerContext.includes(`${lowerEntity}.ts`) || lowerContext.includes(`${lowerEntity}.js`)) {
      return ENTITY_TYPES.FILE;
    }
    if (/^[A-Z][a-z]+[A-Z]/.test(entity)) return ENTITY_TYPES.CLASS;
  ...
  private calculateDecisionImportance(decision: string): number {
    const strongKeywords = ['must', 'critical', 'important', '必须', '关键', '重要'];
    const mediumKeywords = ['should', 'recommend', '应该', '建议'];

    const lower = decision.toLowerCase();
    if (strongKeywords.some((kw) => lower.includes(kw))) return 0.9;
    if (mediumKeywords.some((kw) => lower.includes(kw))) return 0.7;
    return 0.5;
  }

  private inferRelationType(context: string, _e1: string, _e2: string): string {
    const lower = context.toLowerCase();

    if (lower.includes('uses') || lower.includes('使用')) return RELATION_TYPES.USES;
    if (lower.includes('depends') || lower.includes('依赖')) return RELATION_TYPES.DEPENDS_ON;
    if (lower.includes('implements') || lower.includes('实现')) return RELATION_TYPES.IMPLEMENTS;
    if (lower.includes('extends') || lower.includes('继承')) return RELATION_TYPES.EXTENDS;
    if (lower.includes('calls') || lower.includes('调用')) return RELATION_TYPES.CALLS;
