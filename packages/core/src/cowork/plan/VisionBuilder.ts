 ...
 * 默认问题模板
 */
const DEFAULT_QUESTIONS: VisionQuestion[] = [
  {
    id: 'goal-1',
    category: 'goal',
 ...
 * 中文问题模板
 */
const CHINESE_QUESTIONS: VisionQuestion[] = [
  {
    id: 'goal-1',
    category: 'goal',
 ...
 * 默认配置
 */
const DEFAULT_CONFIG: VisionBuilderConfig = {
  maxQuestions: 20,
...
export class VisionBuilder extends EventEmitter {
  private config: VisionBuilderConfig;
  private questions: VisionQuestion[];
  private answers: Map<string, VisionAnswer> = new Map();
  private currentQuestionIndex: number = 0;
  private customQuestions: VisionQuestion[] = [];
  ...
  submitAnswer(answer: string): VisionQuestion | null {
    ...
    const visionAnswer: VisionAnswer = {
   ...
   * 添加后续问题
   */
  private addFollowUpQuestion(category: VisionQuestion['category'], question: string): void {
   ...
   * 从回答构建愿景文档
   */
  private buildVisionDocument(title: string, answers: VisionAnswer[]): VisionDocument {
    const goals: string[] = [];
   ...
   * 查找问题
   */
  private findQuestion(id: string): VisionQuestion | undefined {
