...
import { AtomicMemory } from '../../memory/types.js';
...
const DEFAULT_CONFIG: MemoryShadowIntegrationConfig = {
  shadowRoot: '.codeflow',
  projectRoot: '.',
...
export class MemoryShadowIntegration {
  ...
  private readonly config: MemoryShadowIntegrationConfig;

  constructor(
    hookManager: HookManager,
    memoryService: AtomicMemoryService,
    config: Partial<MemoryShadowIntegrationConfig> = {},
  ...
  register(): void {
    this.hookManager.register(
      ...
      this.onBeforeTaskExecute.bind(this)
    );
    this.hookManager.register(
      HookEvent.AFTER_TASK_EXECUTE,
      this.onAfterTaskExecute.bind(this)
    );
    this.hookManager.register(
      HookEvent.ON_TASK_FAILURE,
      this.onTaskFailure.bind(this)
    );
    this.hookManager.register(
      HookEvent.ON_TASK_COMPLETE,
      this.onTaskComplete.bind(this)
    );
  }
   ...
   */
  private async onBeforeTaskExecute(context: TaskExecutionContext): Promise<void> {
    try {
   ...
   */
  private async onAfterTaskExecute(result: TaskExecutionResult): Promise<void> {
    try {
   ...
   */
  private async onTaskFailure(context: TaskFailureContext): Promise<void> {
    try {
      // 存储失败记忆
      await this.memoryService.add({
      ...
      // 检索历史修复方案
      const historicalFixes = await this.memoryService.search(
        `修复 ${context.error} ${context.title}`,
        {
          tags: ['task_failure'],
   ...
   */
  private async onTaskComplete(result: TaskExecutionResult): Promise<void> {
    try {
      // 存储完成记忆
   ...
   * 加载与任务相关文件的意图文档
   */
  private async loadRelevantIntentDocs(context: TaskExecutionContext): Promise<string[]> {
    const docs: string[] = [];
   ...
   * 检索与任务描述语义相关的原子记忆
   */
  private async searchRelevantMemories(context: TaskExecutionContext): Promise<AtomicMemory[]> {
   ...
   * 构建任务执行记忆内容
   */
  private buildTaskMemoryContent(result: TaskExecutionResult): string {
    const parts = [
   ...
   * 构建任务记忆标签
   */
  private buildTaskMemoryTags(result: TaskExecutionResult): string[] {
   ...
   * 将源文件路径映射到意图文档路径
   */
  private resolveIntentDocPath(sourceFile: string): string {
   ...
   * 对修改的文件重新投影意图文档
   */
  private async syncIntentDocs(filesModified: string[]): Promise<void> {
    if (!this.batchProjector) return;
