    ...
    >
      ...
      >
        <span style={{ fontSize: 12, fontWeight: 600, color: '#333' }}>
          Related memories ({matches.length})
        </span>
        {onClose && (
          <button
            onClick={onClose}
 ...
 * 匹配预览项组件
 */
const MatchPreviewItemComponent: React.FC<{
  match: MatchPreviewItem;
  onClick?: () => void;
}> = ({ match, onClick }) => {
 ...
 * 输入框包装器（带边框变色）
 */
export const MemoryInputWrapper: React.FC<MemoryInputWrapperProps> = ({
  children,
  matchLevel,
  matchCount = 0,
  showBadge = true,
  pulseOnHighMatch = true,
  className,
  style,
  onClick,
}) => {
