...
// Cytoscape 类型（运行时动态加载）
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type CytoscapeInstance = any;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type CytoscapeEvent = any;

/**
 * 将 GraphData 转换为 Cytoscape 元素格式
 */
function toCytoscapeElements(data: GraphData): unknown[] {
...
export const GraphView: React.FC<GraphViewProps> = ({
  ...
  const config = useMemo(
    () => ({ ...DEFAULT_VIEW_CONFIG, ...userConfig }),
    [userConfig]
  );
  ...
  const chunkConfig = useMemo(
    () => ({
      ...DEFAULT_CHUNK_CONFIG,
      onChunkLoaded: (loaded: number, total: number) => {
