...
func TestSQLiteContextServicePresetPersistence(t *testing.T) {
	...
	req := &PresetCreateRequest{
		Name:        "frontend core",
		Description: "主前端上下文",
		Paths:       []string{"/workspace/apps/workbench", "/workspace/backend"},
	...
	req.Paths[0] = "/mutated"
	req.Extensions[0] = "md"
	if created.Paths[0] != "/workspace/apps/workbench" {
	...
	if !reflect.DeepEqual(got.Paths, []string{"/workspace/apps/workbench", "/workspace/backend"}) {
