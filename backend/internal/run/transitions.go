...
//
// Consequence events (run.completed, run.failed, approval.decided,
// budget.warning, merge.completed, tool.requested) are never triggers: they are
// emitted *because* of a transition, and asking NextStatus for one is a
// programming error reported as invalid_transition.
...
// "cleaned" conclusion. A caller that sets an irrelevant field gets
// invalid_transition rather than a silent match, so a caller bug cannot hide
// behind a lenient matcher.
type Trigger struct {
	...
	ExitCode *int
	// ExpectedTerminal is the terminal state a "cleaned" conclusion ends in
	// (failed, cancelled or expired); it is consulted only by the cleaned rows.
...
// transitionRules is the §21.1 table, row by row. Condition and FailureCode are
// the plan's own columns; FailureCode "" means the plan writes "—" (no failure
// code for that row).
var transitionRules = []transitionRule{
	...
	// queued/starting/running/waiting_approval/paused --run.cancel or
	// hard deadline--> cancelling. queued has no process; see the cleaned rows
	// below for how such a Run finishes.
	{
		From: RunStatusQueued, Kind: TriggerCommand, Name: CommandRunCancel,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	},
	{
		From: RunStatusStarting, Kind: TriggerCommand, Name: CommandRunCancel,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	...
	{
		From: RunStatusRunning, Kind: TriggerCommand, Name: CommandRunCancel,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	...
	{
		From: RunStatusWaitingApproval, Kind: TriggerCommand, Name: CommandRunCancel,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	...
	{
		From: RunStatusPaused, Kind: TriggerCommand, Name: CommandRunCancel,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	...
	{
		From: RunStatusQueued, Kind: TriggerCommand, Name: CommandHardDeadline,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	...
	{
		From: RunStatusStarting, Kind: TriggerCommand, Name: CommandHardDeadline,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	...
	{
		From: RunStatusRunning, Kind: TriggerCommand, Name: CommandHardDeadline,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	...
	{
		From: RunStatusWaitingApproval, Kind: TriggerCommand, Name: CommandHardDeadline,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	...
	{
		From: RunStatusPaused, Kind: TriggerCommand, Name: CommandHardDeadline,
		To: RunStatusCancelling,
		Condition: "记录期望终态 cancelled 或 expired；queued 无进程可直接结束 " +
			"(record the expected terminal cancelled/expired; a queued Run has no process to kill)",
		FailureCode: CodeForbidden,
	},
	// cancelling --process.terminated--> cancelled
	{
		From: RunStatusCancelling, Kind: TriggerEvent, Name: string(EventProcessTerminated),
		To:          RunStatusCancelled,
		Condition:   "grace/force 记录齐全 (grace/force records complete)",
	...
	// recovering
	{
		From: RunStatusStarting, Kind: TriggerEvent, Name: string(EventServerRestart),
		To: RunStatusRecovering,
		Condition: "验证 owner instance、PID 启动时间和后端重连能力 " +
			"(verify owner instance, PID start time and backend re-attach capability)",
		FailureCode: CodeRecoveryRequired,
	},
	{
		From: RunStatusRunning, Kind: TriggerEvent, Name: string(EventServerRestart),
		To: RunStatusRecovering,
		Condition: "验证 owner instance、PID 启动时间和后端重连能力 " +
			"(verify owner instance, PID start time and backend re-attach capability)",
		FailureCode: CodeRecoveryRequired,
	...
	{
		From: RunStatusWaitingApproval, Kind: TriggerEvent, Name: string(EventServerRestart),
		To: RunStatusRecovering,
		Condition: "验证 owner instance、PID 启动时间和后端重连能力 " +
			"(verify owner instance, PID start time and backend re-attach capability)",
		FailureCode: CodeRecoveryRequired,
	...
	{
		From: RunStatusPaused, Kind: TriggerEvent, Name: string(EventServerRestart),
		To: RunStatusRecovering,
		Condition: "验证 owner instance、PID 启动时间和后端重连能力 " +
			"(verify owner instance, PID start time and backend re-attach capability)",
		FailureCode: CodeRecoveryRequired,
	...
	{
		From: RunStatusCancelling, Kind: TriggerEvent, Name: string(EventServerRestart),
		To: RunStatusRecovering,
		Condition: "验证 owner instance、PID 启动时间和后端重连能力 " +
			"(verify owner instance, PID start time and backend re-attach capability)",
		FailureCode: CodeRecoveryRequired,
	...
	{
		From: RunStatusRecovering, Kind: TriggerConclusion, Name: ConclusionCleaned,
		Terminal:  RunStatusFailed,
		To:        RunStatusFailed,
		Condition: "仅 verified attach 才 running，否则明确清理/终止 (explicit cleanup/termination otherwise)",
	},
	{
		From: RunStatusRecovering, Kind: TriggerConclusion, Name: ConclusionCleaned,
		Terminal:  RunStatusCancelled,
		To:        RunStatusCancelled,
		Condition: "仅 verified attach 才 running，否则明确清理/终止 (explicit cleanup/termination otherwise)",
	},
	{
		From: RunStatusRecovering, Kind: TriggerConclusion, Name: ConclusionCleaned,
		Terminal:  RunStatusExpired,
		To:        RunStatusExpired,
		Condition: "仅 verified attach 才 running，否则明确清理/终止 (explicit cleanup/termination otherwise)",
	},
	...
	{
		From: RunStatusCancelling, Kind: TriggerConclusion, Name: ConclusionCleaned,
		Terminal: RunStatusCancelled,
		To:       RunStatusCancelled,
		Condition: "queued 无进程可直接结束：进程树确认为空后按期望终态结束 " +
...
func FailureCodeFor(t Trigger) string {
	...
	if t.ExpectedTerminal != "" && !isCleaned {
		return CodeInvalidTransition
	}
	if isExited {
...
// rule, 1 for the non-zero rule) and the cleaned rows carry their terminal, so
// the returned trigger is directly usable.
...
func AllTriggers() []Trigger {
	return []Trigger{
		...
		EventTrigger(string(EventProcessTerminated)),
...
func NextStatus(from RunStatus, t Trigger) (RunStatus, error) {
	if !from.Valid() {
		return from, &TransitionError{From: from, Trigger: t, Code: CodeInvalidTransition}
	}
	for _, r := range transitionRules {
		if r.matches(from, t) {
			return r.To, nil
	...
	return from, &TransitionError{From: from, Trigger: t, Code: FailureCodeFor(t)}
}
