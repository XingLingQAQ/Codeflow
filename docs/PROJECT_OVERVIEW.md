...
> 当前默认主前端为 `apps/workbench`（React + Tauri 双模式；历史 `codeflow_template` 已删除；G01 rename 完成）。`packages/gui` 为历史 GUI 组件资产，`codeflow_extracted` 为迁移中遗留前端，不再作为默认交付入口。
...
- **前端**：React + TypeScript（默认主前端为 `apps/workbench`，支持浏览器与 Tauri/sidecar 双模式）
