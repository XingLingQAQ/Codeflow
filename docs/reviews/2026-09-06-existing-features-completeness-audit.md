...
> 合并状态：2026-09-07，以下 14 项已纳入 [3.0 问题文档 §16](../design/codeflow-3.0/2026-09-03-project-issues-deep-dive.md)、[设计文档 §18](../design/codeflow-3.0/codeflow-3.0-redesign.md) 和 [实施计划 S13/§31](../design/codeflow-3.0/2026-09-03-codeflow-3.0-implementation-plan.md)。下文保留审查时的记录；修复状态统一维护在三份 3.0 文档中，不代表问题已经修复。
...
> 本轮记录 14 个独立问题。初次审查独立记录 E 编号；合并到 3.0 后继续沿用 E-01–E-14，原 I-01–I-70 不变，修复责任与进度以 3.0 实施计划为准。
...
- 证据：[workflow handler](../../backend/internal/api/handlers/workflow.go#L57)、[GetReplay](../../backend/internal/workflow/service.go#L311)、[按会话取审计](../../backend/internal/workflow/service.go#L493)。服务读取项目后，直接对调用者给出的 sessionID 取 trace；没有检查它是否在该项目的 sessionIDs 中。审计也只按 sessionID 过滤。
- 触发：A、B 两项目各有会话，在 A 的 replay URL 传 B 的 sessionID。返回结构仍标 projectID=A，trace/部分审计却来自 B；A 的 Flow 事件还会一起合并，形成混合回放。
- 同域缺口：[loadRelevantAgents](../../backend/internal/workflow/service.go#L475) 在项目没有任何 sessionID 时返回全部 Agent，又把其 sessionID 加回项目快照。旧数据或未形成会话关联的项目会因此扩大统计范围。
...
- 证据：[Update](../../backend/internal/memory/atomic_service.go#L390) 先提交 SQL UPDATE，随后 [replaceVector](../../backend/internal/memory/atomic_service.go#L648) 先删除旧向量再 Add 新向量；[Delete](../../backend/internal/memory/atomic_service.go#L451) 先删除正文，之后才删向量。
- 生产接线：[main](../../backend/cmd/codeflow-server/main.go#L154) 初始化正文和向量的两个 SQLite 文件；不是只存在于测试里的 service。
...
- 证据：[registry.Update](../../backend/internal/skill/registry.go#L234) 先 `store.put(s)` 提交新当前版本，再 [archiveVersionLocked](../../backend/internal/skill/registry.go#L246) 保存旧版。底层 [sqliteSkillStore](../../backend/internal/skill/sqlite_store.go#L86) 的当前写入、归档和裁剪均为独立 Exec。
...
- 证据：[Claude Stream](../../backend/internal/adapters/claude.go#L487)、[OpenAI Stream](../../backend/internal/adapters/openai.go#L129)、[Gemini Stream](../../backend/internal/adapters/gemini.go#L107) 均在 scanner 循环结束后无条件追加 assistant 历史、发送 Done=true，并通知 FinishReason="stop"。
- 三者没有检查 scanner.Err；JSON 解析失败直接 continue；公开的 [StreamChunk](../../backend/internal/adapters/types.go#L63) 只有 Delta/Index/Done，没有异步错误或真正终止原因。
...
- 证据：[OpenAI 发送 chunk](../../backend/internal/adapters/openai.go#L157)、[Gemini](../../backend/internal/adapters/gemini.go#L129)、[Claude](../../backend/internal/adapters/claude.go#L515) 都直接 `ch <- chunk`，最终 Done 发送也一样。
...
- 证据：[main](../../backend/cmd/codeflow-server/main.go#L245) 给生产服务注入 `NewSummarizerService()`；[HTTP handler](../../backend/internal/api/handlers/summarize.go#L14) 直接调用它。
- [SummarizeConversation](../../backend/internal/summarize/service.go#L37) 计算 preserveRecent 后不使用；[generateSummary](../../backend/internal/summarize/service.go#L192) 只是“消息数量 + 最多三条英文关键词句子”；compression_target 直接原样回填到 CompressionRatio，并非测量结果。
- `CompressRequest.target_tokens` 在 [类型](../../backend/internal/summarize/types.go#L53) 中公开，但 CompressContext 不读取它。相同内容设置不同 target_tokens 不会改变该控制路径。
- 仓库另有 [Compressor](../../backend/internal/summarize/compressor.go#L41)，支持消息级策略及可选模型摘要；不能因此宣称 HTTP 接口已经接上该实现。
...
- 证据：[splitPoint](../../backend/internal/summarize/service.go#L78) 按 len(string) 直接切分；[summarizeText](../../backend/internal/summarize/service.go#L211) 按计算出的字节下标截断。类型和 handler 没有比例区间校验。
...
- 证据：[RollbackVersion](../../backend/internal/skill/registry.go#L348) 用 append([]string(nil), snap.Triggers...) 构造更新；旧版本为空时结果为 nil。随后 [Update](../../backend/internal/skill/registry.go#L223) 仅在字段非 nil 时修改，StageTags 同样如此。
...
- 证据：[DefinePAPIVariable](../../backend/internal/config/service.go#L579)、DeletePAPIVariable、HotSwapPAPI 先改 papiManager，再单独调用持久化函数；只有数据库写函数内部持有 s.mu，整个操作没有共同串行边界。
...
- 证据：[ResolveByCategory](../../backend/internal/config/papi.go#L103) 遍历 Go map 并 EqualFold；[DetectConflicts](../../backend/internal/config/papi.go#L205) 则用原始 category 字符串作 map key。
...
- 证据：[Search](../../backend/internal/memory/atomic_service.go#L165) 向量查询只取 limit+offset（默认 10），仅提前传 session 过滤；[matchAtomicFilters](../../backend/internal/memory/atomic_service.go#L730) 对 folder/tags/time 的过滤在候选截断之后执行。
...
- 证据：[ApplyHeatDecay](../../backend/internal/memory/atomic_service.go#L531) 对 stmt.ExecContext 的 result/error 均未处理，最后返回 len(items)，而不是已成功更新数量。
...
- 证据：[Settings](../../apps/workbench/src/shell/pages/Settings.tsx#L33) 中 autoSnapshot/livePreview 都是组件 useState，唯一消费者是 [Switch](../../apps/workbench/src/shell/pages/Settings.tsx#L141)。未接 store、后端配置或功能判断。
...
- 证据：[execGit](../../backend/internal/git/manager.go#L51) 对整个输出 TrimSpace；[Status](../../backend/internal/git/manager.go#L87) 再按 porcelain 固定列取 line[:2] 和 line[3:]。
- 常见触发：首行是未暂存修改 ` M foo.go`，TrimSpace 后成为 `M foo.go`，固定列提取得到文件名 `oo.go`，首字符丢失。
- 第二处证据：[DiffBetween](../../backend/internal/git/manager.go#L449) 用 strings.Fields 拆文件名，且只识别状态字面值 R，不能正确处理 R100/R087 以及旧/新两个路径。带空格路径也会被截断。
